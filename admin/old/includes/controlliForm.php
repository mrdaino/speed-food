...
function creaElementi($tipo,$nomeDb,$nomeTr,$descrizione,$n,$arrValue="",$arrValueDesc=""){
	...
	switch ($tipo) {
	           ...
	           $id=$nomeDb;
				echo "
					<tr  id='".$id."'>
						...
						for($j=0;$j<$i; $j++){
	                    	...
	                    	echo"	 id='".$id."img".$j."' src='".LOCAL."/img/".$immagini[$j]."'>";
	                    	if(($j)>floor($array[$z]->fileMinNumber)||$array[$z]->obbligatorio==false)
									echo"<br/><img class='delP' onclick='eliminaFiles(\"".$nomeDb."\",\"".$template."\",\"".$immagini[$j]."\",\"".$row[$idDelImg]."\",\"img\")' src='".LOCAL."/admin/img/elimina.png'>";
	                		echo"
	                		</div>
	                		...
	                		<script>numImg++; $(document).on('change','#".$id."imgF".$j."',function(event){caricaImg('".$id."imgF".$j."','".$j."','".$id."'";
	                			if($arrValue!="") echo ",'$arrValue'";
	                			if($arrValueDesc!="") echo ",'$arrValueDesc'";
	                		echo");});</script>
	                		";
						}
				...
				echo"	</td>
						<td>
							...
							echo"<p id='".$id."numaggImg' style='display:none;'>".($j)."</p>
							<span class='aggSpan intMenu' title='Aggiungi ".$nomeTr."' onclick='aggImg(\"".$id."\" ";
								if($arrValue!="") echo ',"'.$arrValue.'"';
	                			if($arrValueDesc!="") echo ',"'.$arrValueDesc.'"';
							echo")' style='font-size: 21px; top: 3px; cursor:pointer'>+</span>
						</td>
					</tr>";
		...
		case 'fileFisso':
			...
			for($j=0;$j<$i; $j++){
				echo
			    "<a target='blank' ";if($files[$j]=="") echo" style='display:none;'";
			    echo"id='".$id."fileA".$j."' href='../../../files/".$files[$j]."'><img class='fileP' title='".$files[$j]."' src='../../img/file.png'></a>".
				"<input type='file' class='file_input' name='".$id."fileF".$j."' id='".$id."fileF".$j."' value='".$files[$j]."'/>".
...
function creaElementiRiep($tipo,$nomeDb,$id){
	global $row;
	global $template;
	global $array;
	/*if(get_magic_quotes_gpc()) {
	...
	*/
	switch ($tipo) {
		case 'antTitolo':
			...
			else echo"<td>Il titolo è privato</td>";
			return sprintf("%s",$video->getTitle());
		break;
		case 'antImg':
			require_once(dirname(__FILE__) ."../../../import/youtube.php");
			$video = YoutubeDataApi::getVideoResponse($row[$nomeDb]);
			...
			if($imgs[0]['url']=="")
					echo"<td><img class='imgPrev' src='../../../img/video/default_thumb.jpg'></td>";
			else	echo"<td><img class='imgPrev' src='".$imgs[0]['url']."'></td>";	
		break;
		case 'join_tipo':
			echo"<td>".get_tipo($row[$nomeDb])."</td>";
		break;
		case 'join':
			...
			echo"<td>".($iscritti)."/".$row['pro_max_iscritti']." max <br/> <a href='iscritti_provini.php?pro_id=".$row[$nomeDb]."&titolo=".$row['pro_titolo']."'>Iscritti</a></td>";
		break; 
		case 'img':
			...
			for($i=0;$i<count($arrImg)-1;$i++){
				echo"<div class='divImgP' >
						<img class='imgPrev' title='".$arrImg[$i]."' src='../../../img/".$arrImg[$i]."'>
						<br/>";
						if(($i)>floor($array[$j]->fileMinNumber)||$array[$j]->obbligatorio==false)
						echo"<img class='delP' onclick='eliminaFiles(\"".$nomeDb."\",\"".$template."\",\"".$arrImg[$i]."\",\"".$id."\",\"img\")' src='".LOCAL."/admin/img/elimina.png'>";
				echo"	</div>";
			}
			echo"</td>";
			break;
		case 'data':
			echo"<td>".date("d-m-Y",strtotime($row[$nomeDb]))."</td>";
		break;
		case 'int':;
		case 'txt':
			echo"<td>".substr($row[$nomeDb],0,100)."</td>";
			break;
		case 'textarea':
			echo"<td>".substr(strip_tags($row[$nomeDb]),0,100)."</td>";
			break;
		case 'select':
			echo"<td>".substr(strip_tags($row[$nomeDb]),0,100)."</td>";
			break;
			...
			for($i=0;$i<count($arrImg)-1;$i++){
				echo"
					<div class='divImgP' >
						...
						<br/>";
						if(($i)>floor($array[$j]->fileMinNumber)||$array[$j]->obbligatorio==false)
						echo"<img class='delP' onclick='eliminaFiles(\"".$nomeDb."\",\"".$template."\",\"".$arrImg[$i]."\",\"".$id."\",\"file\")' src='".LOCAL."/admin/img/elimina.png'>";
					echo"</div>";
			}		
			echo"</td>";
			break;
		case 'radio':
			 echo"<td>"; 
			 	if(floor($row[$nomeDb])==1) echo "Artista In Esclusiva";  
			 	else echo "Artista In Distribuzione"; 
			 echo"</td>";
			break;
		default:
			# code...
		...
		if(isset($_POST[$v[$i]->nomePost])){
			...
			if(!$v[$i]->obbligatorio||$_POST[$v[$i]->nomePost]!=""){
				if($_POST[$v[$i]->nomePost]!=""){
					//---------------
						if($v[$i]->nomeDB!="mostraHtml"){
							$_POST[$v[$i]->nomePost]=strip_tags($_POST[$v[$i]->nomePost],"<p><b><em><table><tbody><tr><td><ol><li><strong><br>");
						...
						if(strcmp($nome, $v[$i]->nomeDB."imgF")==0){
							...
							//todo
								...
								*/
								...
								$fullPath=$v[$i]->percorsoFile."/".$nomeimmagine[0]."_".$indice.".".$ext;
								...
								/*if(!move_uploaded_file($_FILES[$nome.$j]['tmp_name'], $path."img/".$v[$i]->percorsoFile."/img_".$indice."_".$v[$i]->percorsoFile."FullSize.".$ext)){

								   return "Si &egrave; verificato un errore durante il salvataggio del file";
								}
								*/
								$nomeimmagine=preg_split("/.".$ext."/", strtolower($_FILES[$nome.$j]['name']));
								$fullsize=$v[$i]->percorsoFile."/".$nomeimmagine[0]."_".$indice."FullSize.".$ext;
								$fullsizeGif=$v[$i]->percorsoFile."/".$nomeimmagine[0]."_".$indice."asd.".$ext;
								if($ext!="gif"){ 
									...
									imagepng($dstImg, $path."img/".$fullPath,9);
									imagedestroy($dstImg);
								}
								else{
									...
									imagecopyresampled($dstImg, $vImg, 0, 0, $x, $y, $nw, $nh, $w, $h);
									//imagejpeg($dstImg, $path."img/".$v[$i]->percorsoFile."/img_".$indice."_".$v[$i]->percorsoFile.".".$ext);
									imagepng($dstImg, $path."img/".$fullsize,9);
									imagedestroy($dstImg);
								}
								//$immagini.=$v[$i]->percorsoFile."/img_".$indice."_".$v[$i]->percorsoFile.".".$ext.";";
								//$v[$i]->value.=$v[$i]->percorsoFile."/img_".$indice."_".$v[$i]->percorsoFile.".".$ext.";";
								$immagini.=$fullPath.";";
							...
							$fullPath=$v[$i]->percorsoFile."/";
							$fullPath.=safe_filename(strtolower($_FILES[$nome.$j]['name']));
							/*while (file_exists($path."files/".$v[$i]->percorsoFile."/file_".$indice."_".$v[$i]->percorsoFile.".".$ext)) {
								$indice++;
							}*/
							while (file_exists($path."files/".$fullPath)) {
								$fullPath=$v[$i]->percorsoFile."/".basename(strtolower($fullPath),".".$ext)."_".$indice.$ext;
								$indice++;
							}
							if($update!="" && $fileS[$j]!=""){
								unlink("../../../../files/".$fileS[$j]);
							}
							/*if(!move_uploaded_file($_FILES[$nome.$j]['tmp_name'], $path."files/".$v[$i]->percorsoFile."/file_".$indice."_".$v[$i]->percorsoFile.".".$ext)){
							...
							/*$files.=$v[$i]->percorsoFile."/file_".$indice."_".$v[$i]->percorsoFile.".".$ext.";";
							$v[$i]->value.=$v[$i]->percorsoFile."/file_".$indice."_".$v[$i]->percorsoFile.".".$ext.";";*/
							$files.=$fullPath.";";
