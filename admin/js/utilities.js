...
$(document).ready(function(){
	...
	$(".title").find("span").click(function(){
		global_menu=$(this).parent().find(".title_div");
		$(this).parent().find(".title_div").slideToggle();
		
	});

	var pathname = window.location.pathname;
	var res = pathname.split("/");
	var indU = res.indexOf("utilities");
	if(res[indU+1] !== undefined)
		$('#menu_left a[href*="'+res[indU+1]+'"]').parent().parent().slideToggle();
	// end
	//smooth scroll
...
function aggImg_not_resize(id,lg,al){
	if (lg === undefined) lg="";
	if (al === undefined) al="";
	num=parseInt($("#"+id+"numaggImg").text());
	var prova=id;
	...
	if(parseInt(num)<array2[i].fileMaxNumber){
		...
		$("#td_"+id).append(""+
        	...
        	"	 <img style='display:none;' class='imgP' id='"+id+"img"+num+"' src=''>"+
    		"</div>"+
    		"<script> $('#"+id+"imgF"+num+"').click(); $(document).on('change','#"+id+"imgF"+num+"',function(event){caricaImg_not_resize('"+id+"imgF"+num+"','"+num+"','"+id+"');});</script>"
	    );
		//$("#"+id+"numaggImg").text(num);
	}else{
		alert("Puoi inserire fino ad un massimo di "+array2[i].fileMaxNumber+" immagine/i");
...
function caricaImg_not_resize(id,nimg,idP){
		var modificato=false;
		$("#valNumImg").val(nimg);
		$("#idChangeImg").val(idP);
	    var oFReader = new FileReader();
    	...
    	if(tipo[0]=="image"){
		    ...
		    oFReader.onload = function (evt) {    
		    	$('#'+idP+'img'+nimg).attr('src', evt.target.result).fadeIn();
		    	if(!modificato) $("#"+idP+"numaggImg").text(parseInt($("#"+idP+"numaggImg").text())+1);
	    	};
		}else{
	    	alert("Il file caricato non è un'immagine");
	    }
